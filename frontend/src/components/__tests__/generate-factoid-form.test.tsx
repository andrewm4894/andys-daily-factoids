...
const mockGenerateFactoid = api.generateFactoid as jest.MockedFunction<typeof api.generateFactoid>
const mockCreateCheckoutSession = api.createCheckoutSession as jest.MockedFunction<typeof api.createCheckoutSession>
...
describe('GenerateFactoidForm', () => {
  ...
  beforeEach(() => {
    ...
    mockGenerateFactoid.mockResolvedValue(undefined as any)
  ...
  describe('Basic Rendering', () => {
    it('should render the form with generate button', () => {
      ...
      expect(screen.getByRole('button', { name: /Generate factoid/i })).toBeInTheDocument()
    ...
    it('should render shuffle button when onShuffle prop is provided', () => {
      ...
      expect(screen.getByRole('button', { name: /Shuffle factoids/i })).toBeInTheDocument()
    })

    it('should not render shuffle button when onShuffle prop is not provided', () => {
      render(<GenerateFactoidForm {...defaultProps} />)
      
      expect(screen.queryByRole('button', { name: /Shuffle factoids/i })).not.toBeInTheDocument()
    })

    it('should render shuffle loading state', () => {
      const onShuffle = jest.fn()
      render(<GenerateFactoidForm {...defaultProps} onShuffle={onShuffle} shuffleLoading={true} />)
      
      expect(screen.getByRole('button', { name: /Shuffling/i })).toBeInTheDocument()
    })
  })
  ...
  describe('Advanced Options', () => {
    ...
    it('should update topic and model state when inputs change', async () => {
      ...
      await waitFor(() => {
        ...
        fireEvent.change(topicInput, { target: { value: 'space exploration' } })
  ...
  describe('Form Submission', () => {
    it('should call generateFactoid with no parameters when EventSource is not available', async () => {
      ...
      const submitButton = screen.getByRole('button', { name: /Generate factoid/i })
    ...
    it('should disable buttons during generation', async () => {
      ...
      mockGenerateFactoid.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)))
      
      render(<GenerateFactoidForm {...defaultProps} onShuffle={jest.fn()} />)
      
      const submitButton = screen.getByRole('button', { name: /Generate factoid/i })
      const shuffleButton = screen.getByRole('button', { name: /Shuffle factoids/i })
  ...
  describe('EventSource Streaming', () => {
    ...
    it('should handle error events', async () => {
      ...
      render(<GenerateFactoidForm {...defaultProps} onGenerationError={onGenerationError} />)
  ...
  describe('Accessibility', () => {
    it('should have proper form labels and ARIA attributes', async () => {
      ...
      await waitFor(() => {
        const topicInput = screen.getByLabelText('Topic (optional)')
        const modelSelect = screen.getByLabelText('Model (optional)')
        const advancedToggle = screen.getByRole('button', { name: /Hide options/i })
        
        expect(topicInput).toHaveAttribute('id', 'topic')
        expect(modelSelect).toHaveAttribute('id', 'model')
        expect(advancedToggle).toHaveAttribute('aria-expanded', 'true')
        expect(advancedToggle).toHaveAttribute('aria-controls', 'generate-factoid-options')
      })
    })
    ...
    it('should have proper button titles and tooltips', () => {
      ...
      const generateButton = screen.getByRole('button', { name: /Generate factoid/i })
      const shuffleButton = screen.getByRole('button', { name: /Shuffle factoids/i })
      
      expect(generateButton).toHaveAttribute('title', 'Generate a factoid - press show options to pick topic and model')
      expect(shuffleButton).toHaveAttribute('title', 'Randomly sample a different batch')
    })
  ...
  describe('Stripe Integration', () => {
    ...
    it('should show redirecting state during checkout', async () => {
      ...
      mockGenerateFactoid.mockRejectedValueOnce(rateLimitError)
      mockCreateCheckoutSession.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)))
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: /Redirecting to checkout/i })).toBeInTheDocument()
      })
    })
  })
})
