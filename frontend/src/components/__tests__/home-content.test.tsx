...
jest.mock("../generate-factoid-form", () => ({
  GenerateFactoidForm: ({
    ...
    <div data-testid="generate-factoid-form">
      ...
      <button onClick={() => onGenerationError(null)}>Clear Error</button>
...
describe("HomeContent", () => {
  ...
  describe("Initial Rendering", () => {
    ...
    it("should show empty state when no factoids", () => {
      ...
      expect(
        screen.getByText("No factoids yet. Generate one to get started!")
      ).toBeInTheDocument();
      expect(
    ...
    it("should not show error message initially", () => {
      ...
      expect(
        screen.queryByText("Factoid generation failed")
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("FactoidCard Rendering", () => {
    it("should render factoids with correct alternation and color indices", () => {
      ...
      expect(
        firstCard.querySelector('[data-testid="is-alternate"]')
      ).toHaveTextContent("false");
      expect(
        firstCard.querySelector('[data-testid="color-index"]')
      ).toHaveTextContent("0");

      // Second factoid (index 1): alternate, color index 1
      const secondCard = screen.getByTestId("factoid-card-factoid-2");
      expect(
        secondCard.querySelector('[data-testid="is-alternate"]')
      ).toHaveTextContent("true");
      expect(
        secondCard.querySelector('[data-testid="color-index"]')
      ).toHaveTextContent("1");

      // Third factoid (index 2): not alternate, color index 2
      const thirdCard = screen.getByTestId("factoid-card-factoid-3");
      expect(
        thirdCard.querySelector('[data-testid="is-alternate"]')
      ).toHaveTextContent("false");
      expect(
        thirdCard.querySelector('[data-testid="color-index"]')
      ).toHaveTextContent("2");
    });
  ...
  describe("Initial Factoids Update", () => {
    it("should update factoids when initialFactoids prop changes", () => {
      const { rerender } = render(
        <HomeContent initialFactoids={[mockFactoids[0]]} models={mockModels} />
      );

      expect(screen.getByTestId("factoid-card-factoid-1")).toBeInTheDocument();
      expect(
        screen.queryByTestId("factoid-card-factoid-2")
      ).not.toBeInTheDocument();
    ...
    it("should show empty state when initialFactoids becomes empty", () => {
      ...
      expect(
        screen.queryByText("No factoids yet. Generate one to get started!")
      ).not.toBeInTheDocument();
  ...
  describe("Error Handling", () => {
    it("should show error message when generation fails", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("Factoid generation failed")
        ).toBeInTheDocument();
  ...
  describe("Shuffle Functionality", () => {
    it("should pass shuffle loading state to GenerateFactoidForm", async () => {
      const newFactoids = [
        createMockFactoid({ id: "new-factoid", text: "New factoid" }),
      ];
    ...
    it("should not shuffle if already shuffling", async () => {
      mockFetchRandomFactoids.mockImplementation(
  ...
  describe("Local Shuffle Algorithm", () => {
    it("should shuffle factoids randomly", async () => {
      ...
      // Get initial order
      const initialCards = manyFactoids.map((f) =>
        screen.getByTestId(`factoid-card-${f.id}`)
      );
      ...
      // Get new order
      const shuffledCards = manyFactoids.map((f) =>
        screen.getByTestId(`factoid-card-${f.id}`)
      );
      const shuffledTexts = shuffledCards.map(
        (card) =>
          card.querySelector('[data-testid="factoid-text"]')?.textContent
      );
  ...
  describe("Component Integration", () => {
    ...
    it("should pass correct props to FactoidCard components", () => {
      ...
      mockFactoids.forEach((factoid, index) => {
        ...
        expect(
          card.querySelector('[data-testid="factoid-text"]')
        ).toHaveTextContent(factoid.text);
        expect(
          card.querySelector('[data-testid="is-alternate"]')
        ).toHaveTextContent((index % 2 === 1).toString());
        expect(
          card.querySelector('[data-testid="color-index"]')
        ).toHaveTextContent((index % 6).toString());
      });
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle factoids with missing properties gracefully", () => {
      ...
      render(
        <HomeContent
          initialFactoids={[incompleteFactoid]}
          models={mockModels}
        />
      );
      ...
      expect(
        screen
          .getByTestId("factoid-card-incomplete")
          .querySelector('[data-testid="factoid-text"]')
      ).toHaveTextContent("");
    });
    ...
    it("should handle very long factoid texts", () => {
      ...
      render(
        <HomeContent initialFactoids={[longTextFactoid]} models={mockModels} />
      );

      expect(screen.getByTestId("factoid-card-long-text")).toBeInTheDocument();
      expect(
        screen
          .getByTestId("factoid-card-long-text")
          .querySelector('[data-testid="factoid-text"]')
