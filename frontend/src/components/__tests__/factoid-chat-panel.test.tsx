...
>;
const mockIsChatRateLimitError =
  api.isChatRateLimitError as jest.MockedFunction<
    typeof api.isChatRateLimitError
  >;
...
describe("FactoidChatPanel", () => {
  ...
  describe("Initial Rendering", () => {
    ...
    it("should render close button with proper accessibility", async () => {
      ...
      await renderWithAct(
        <FactoidChatPanel factoid={mockFactoid} onClose={onClose} />
      );
    ...
    it("should render message input with placeholder", async () => {
      ...
      const textarea = screen.getByPlaceholderText(
        "Ask a question, request sources, or say hi"
      );
  ...
  describe("Session Initialization", () => {
    it("should initialize session on mount", async () => {
      ...
      await waitFor(() => {
        expect(mockCreateChatSession).toHaveBeenCalledWith({
          factoidId: "test-factoid-id",
        });
      });

      await waitFor(() => {
        expect(
          screen.getByText("Ask anything about this factoid to get started.")
        ).toBeInTheDocument();
      });
    });
    ...
    it("should reset state when factoid changes", async () => {
      ...
      const { rerender } = await renderWithAct(
        <FactoidChatPanel factoid={mockFactoid} />
      );

      await waitFor(() => {
        expect(
          screen.getByText("This factoid is fascinating because...")
        ).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("This factoid is fascinating because...")
  ...
  describe("Message Display", () => {
    ...
    it("should display messages with correct styling", async () => {
      ...
      await waitFor(() => {
        ...
        const assistantMessage = screen.getByText(
          "This factoid is fascinating because..."
        );
        ...
        expect(userMessage.closest("div")).toHaveClass(
          "self-end",
          "bg-indigo-50",
          "text-indigo-900"
        );
        ...
        expect(assistantMessage.closest("div")).toHaveClass(
          "self-start",
          "bg-slate-100",
          "text-slate-900"
        );
    ...
    it("should display rate limit information", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("3/10 requests used this minute")
        ).toBeInTheDocument();
      });
    });
    ...
    it("should filter out tool messages from display", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Tell me more about this factoid")
        ).toBeInTheDocument();
        expect(
          screen.getByText("This factoid is fascinating because...")
        ).toBeInTheDocument();
  ...
  describe("Rate Limiting", () => {
    it("should handle rate limit errors with checkout session", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("10/10 requests used this minute")
        ).toBeInTheDocument();
